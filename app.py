...
if "Dashboard" in menu:
    ...
    # --- PURE MINIMALIST CARD FOR ACTIVE PROJECT ---
    ...
    if not df.empty:
        ...
        if net_bal < 50000:
            st.markdown(f"""<div style="background-color: #fee2e2; border-left: 5px solid #ef4444; padding: 10px; margin-bottom: 10px; border-radius: 5px;">🚨 RUNNING BALANCE WARNING: Capital pool reserve is critical (PKR {net_bal:,.0f}) inside {current_project}.</div>""", unsafe_allow_html=True)
        elif daily_burn_rate > 0:
            days_left = net_bal / daily_burn_rate
            if days_left <= 5:
                st.markdown(f"""<div style="background-color: #fffbeb; border-left: 5px solid #f59e0b; color: #78350f; padding: 10px; margin-bottom: 10px; border-radius: 5px;">⚠️ RESERVES DEFICIT: Capital status estimated to expire in ~{days_left:.1f} days.</div>""", unsafe_allow_html=True)
            else:
                st.markdown(f"""<div style="background-color: #f0fdf4; border-left: 5px solid #22c55e; padding: 10px; margin-bottom: 10px; border-radius: 5px;">📈 RUNWAY STABILITY PROJECTION: Safe operational buffer mapped: ~{days_left:.1f} Days.</div>""", unsafe_allow_html=True)
...
# --- 10. 📑 RECEIPT VOUCHER SYSTEM ---
    ...
    if not df.empty:
        ...
        selected_voucher_label = st.selectbox("Select Transaction Log Entry:", df['voucher_label'].tolist())
        ...
        st.markdown(f"""
            <div class="digital-voucher">
                <div style="text-align: center; border-bottom: 2px dashed #e2e8f0; padding-bottom: 15px; margin-bottom: 15px;">
                    ...
                    <p style="margin: 4px 0 0 0; font-size: 11px; color: #64748b; font-weight: 600; text-transform: uppercase;">Official Cash Flow Voucher</p>
                </div>
                ...
                <div style="margin-top: 15px; font-size: 12px; color: #64748b; font-style: italic; border-top: 1px solid #e2e8f0; padding-top: 10px;">
                    <b>Log Notes:</b> {v_row['detail'] if v_row['detail'] else 'No supplementary details.'}
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    else: st.info("No transaction entries available for this project.")


# --- 11. 💰 INCOME LEDGER ---
elif menu == "💰 Income Ledger":
    st.title("💰 Capital Influx & Income Ledger")
    st.write(f"Project context filtering: **{current_project}**")
    
    sub_df = df[df['type'] == 'Income'] if not df.empty else pd.DataFrame()
    st.metric("Total Income Captured", f"PKR {sub_df['amount'].sum() if not sub_df.empty else 0:,.0f}")
    
    if not sub_df.empty:
        st.dataframe(sub_df[['id', 'date', 'name', 'amount', 'pay_method', 'received_by', 'detail']], use_container_width=True)
        if is_auth:
            del_id = st.number_input("Delete Income Log ID Entry", min_value=1, step=1)
            if st.button("🗑️ Wipe Income Record"):
                supabase.table('transactions').delete().eq('id', del_id).execute()
                st.cache_data.clear(); st.success("Deleted!"); st.rerun()
    else: st.info("No income data discovered.")


# --- 12. 👷 LABOR LEDGER HISTORY ---
elif menu == "👷 Labor Ledger History":
    st.title("👷 Labor Disburse Capital Ledger")
    sub_df = df[df['type'] == 'Labor'] if not df.empty else pd.DataFrame()
    st.metric("Total Labor Payouts", f"PKR {sub_df['amount'].sum() if not sub_df.empty else 0:,.0f}")
    
    if not sub_df.empty:
        st.dataframe(sub_df[['id', 'date', 'name', 'occupation', 'amount', 'pay_method', 'detail']], use_container_width=True)
        if is_auth:
            del_id = st.number_input("Delete Entry ID", min_value=1, step=1)
            if st.button("🗑️ Wipe Labor Record"):
                supabase.table('transactions').delete().eq('id', del_id).execute()
                st.cache_data.clear(); st.success("Removed!"); st.rerun()
    else: st.info("No labor debit logs posted.")


# --- 13. 🏗️ MATERIAL LOG VAULT ---
elif menu == "🏗️ Material Log Vault":
    st.title("🏗️ Material Supply & Invoice Log Vault")
    sub_df = df[df['type'] == 'Material'] if not df.empty else pd.DataFrame()
    st.metric("Total Material Expenditures", f"PKR {sub_df['amount'].sum() if not sub_df.empty else 0:,.0f}")
    
    if not sub_df.empty:
        for idx, r in sub_df.iterrows():
            with st.container():
                col_t, col_i = st.columns([3, 1])
                col_t.markdown(f"#### ID #{r['id']} - {r['name']} | PKR {r['amount']:,.0f}")
                col_t.write(f"📅 Date: {r['date']} | 💳 Method: {r['pay_method']} | 📝 Details: {r['detail']}")
                if r.get('image_url'):
                    col_i.image(r['image_url'], width=150)
                st.divider()
        if is_auth:
            del_id = st.number_input("Delete Invoice ID", min_value=1, step=1)
            if st.button("🗑️ Delete Material Invoice"):
                supabase.table('transactions').delete().eq('id', del_id).execute()
                st.cache_data.clear(); st.success("Erased!"); st.rerun()
    else: st.info("No material tracking matrices saved.")


# --- 14. 📋 PENDING BILLS HISTORY ---
elif menu == "📋 Pending Bills History":
    st.title("📋 Unpaid Accounts & Pending Bills Tracker")
    sub_df = df[df['type'] == 'Pending Bill'] if not df.empty else pd.DataFrame()
    st.metric("Total Liabilities Stack", f"PKR {sub_df['amount'].sum() if not sub_df.empty else 0:,.0f}")
    
    if not sub_df.empty:
        st.dataframe(sub_df[['id', 'date', 'name', 'amount', 'detail']], use_container_width=True)
        
        st.markdown("### ⚡ Liability Settlement Action Bridge")
        target_id = st.number_input("Target Pending Bill Token ID", min_value=1, step=1)
        action_col1, action_col2 = st.columns(2)
        
        if action_col1.button("✅ Shift to Material Outflows", use_container_width=True):
            update_transaction_status(target_id, "Material")
        if action_col2.button("👷 Shift to Active Labor Outflows", use_container_width=True):
            update_transaction_status(target_id, "Labor")
            
        if is_auth:
            st.divider()
            if st.button("🗑️ Clear Liability Item permanently"):
                supabase.table('transactions').delete().eq('id', target_id).execute()
                st.cache_data.clear(); st.success("Liability Deleted!"); st.rerun()
    else: st.success("Clear Runway! Zero pending accounts payable.")


# --- 15. 👷 LABOR FORCE FOLDER ---
elif menu == "👷 Labor Force Folder":
    st.title("👷 Labor Force Profiles Directory")
    if not labor_df.empty:
        for idx, row in labor_df.iterrows():
            with st.expander(f"👤 Profile: {row['name']} ({row['occupation']})"):
                c1, c2 = st.columns([1, 3])
                if row.get('photo_url'): c1.image(row['photo_url'], width=120)
                c2.markdown(f"**CNIC:** {row['cnic']} | **Phone:** {row['phone']}")
                c2.markdown(f"**Contract Capital Allocation:** PKR {row['total_contract_amount']:,.0f}")
                c2.markdown(f"⭐ **Internal Grading:** {row['rating']}/5")
                c2.write(f"📝 **Profile Ledger Notes:** {row['details']}")
                
                # Fetch paid transactions specifically filtered for this worker name
                w_payments = df[(df['type'] == 'Labor') & (df['name'].str.lower() == row['name'].lower())] if not df.empty else pd.DataFrame()
                w_paid = w_payments['amount'].sum() if not w_payments.empty else 0
                w_remaining = row['total_contract_amount'] - w_paid
                
                k1, k2 = c2.columns(2)
                k1.metric("Disbursed Taka Sum", f"PKR {w_paid:,.0f}")
                k2.metric("Remaining Contract Backlog", f"PKR {w_remaining:,.0f}")
                
                # Print Individual Dossier PDF
                pdf_data = export_labor_profile_pdf(row, w_payments)
                c2.download_button(f"📥 Export Profile Dossier ({row['name']})", data=pdf_data, file_name=f"Labor_Profile_{row['name']}.pdf", mime="application/pdf")
    else: st.info("No active workers linked under this project node context.")


# --- 16. 🔍 SEARCH & AUDIT REPORTS ---
elif menu == "🔍 Search & Audit Reports":
    st.title("🔍 Deep System Ledger Query & Audit Report Export")
    if not df.empty:
        q_term = st.text_input("Global Dynamic Search Query Filter (Title, Particulars, Details)")
        f_df = df[df['name'].str.contains(q_term, case=False, na=False) | df['detail'].str.contains(q_term, case=False, na=False)]
        st.dataframe(f_df, use_container_width=True)
        
        pdf_buf = export_to_pdf(f_df, f"System Diagnostic Export Matrix - {current_project}")
        st.download_button("📥 Print Hardcopy Audit System Ledger (PDF)", data=pdf_buf, file_name=f"Deewaryn_Audit_Report_{current_project}.pdf", mime="application/pdf")
    else: st.info("No query arrays compiled.")
