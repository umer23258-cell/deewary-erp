...
def fetch_project_status():
    try:
        ...
        if not res.data:
            # Agar table khali hai toh ye default tasks show karega
...
if menu == "📊 Dashboard":
    ...
    # Admin Control Panel (Sirf login ke baad dikhega)
    if is_auth:
        with st.expander("🛠️ Admin: Update Work Status"):
            st.write("Yahan se aap kisi bhi kaam ka status badal sakte hain:")
            selected_task = st.selectbox("Kaam Select Karein", status_df['task_name'].tolist())
            new_status = st.radio("Status Badlein", ["Pending", "Done"], horizontal=True)
            
            if st.button("Update Status Now"):
                supabase.table('project_status').upsert({"task_name": selected_task, "status": new_status}).execute()
                st.success(f"{selected_task} status has been updated!")
                st.rerun()
...
# --- 6. HISTORY PAGES (Original logic preserved) ---
else:
    st.title(menu)
    if not df.empty:
        ...
        # (Aapka original edit/delete logic yahan auto-run hoga)
