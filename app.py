...
def fetch_update_comments(update_id):
    try:
        response = (supabase.table('project_update_comments').select('*')
                    .eq('update_id', update_id).order('created_at').execute())
        return pd.DataFrame(response.data)
    except Exception:
        return pd.DataFrame()

@st.cache_data(ttl=300)
def fetch_company_logo_url():
    try:
        rows = (supabase.table('company_settings').select('setting_value')
                .eq('setting_key', 'company_logo').limit(1).execute().data or [])
        return rows[0].get('setting_value') if rows else ''
    except Exception:
        return ''
...
def add_update_comment(update_id, author, body):
    supabase.table('project_update_comments').insert({
        'update_id': update_id,
        'author_name': author.strip() or 'Site visitor',
...
def render_project_updates_slider(updates_df, height=430):
    ...
    for _, item in updates_df.iterrows():
        ...
        slides.append({
            ...
            'mime': str(item.get('media_mime', 'video/mp4' if media_url.lower().split('?')[0].endswith('.mp4') else 'video/webm')),
    ...
    components.html(f'''\
        ...
        * {{ box-sizing:border-box }} body {{ margin:0; font-family:Inter,system-ui,sans-serif; background:#101b35 }}
        ...
        function showSlide(nextIndex) {{
            ...
            if (slide.kind === 'video') {{
                element.controls = true; element.autoplay = true; element.muted = true; element.loop = true; element.playsInline = true; element.preload = 'metadata';
...
def popup_project_update(current_project):
    ...
    uploaded_media = st.file_uploader(
        ...
        type=['jpg', 'jpeg', 'png', 'webp', 'mp4', 'webm'],
    ...
    if save_update:
        ...
        media_mime = 'video/webm' if extension == 'webm' else 'video/mp4' if extension == 'mp4' else uploaded_media.type or 'image/jpeg'
...
@st.dialog("🖼️ Update Company Logo")
def popup_update_company_logo():
    uploaded_logo = st.file_uploader("Upload logo (PNG, JPG, or WebP)", type=['png', 'jpg', 'jpeg', 'webp'])
    if st.button("Save company logo", type="primary", use_container_width=True):
        if not uploaded_logo:
            st.error("Please choose a logo image.")
            return
        safe_name = ''.join(char if char.isalnum() or char in '._-' else '_' for char in uploaded_logo.name)
        path = f"branding/{uuid.uuid4().hex}_{safe_name}"
        try:
            supabase.storage.from_('material_pics').upload(path, uploaded_logo.getvalue(), file_options={"content-type": uploaded_logo.type})
            logo_url = supabase.storage.from_('material_pics').get_public_url(path)
            supabase.table('company_settings').upsert({'setting_key': 'company_logo', 'setting_value': logo_url}, on_conflict='setting_key').execute()
            st.cache_data.clear()
            st.success("Company logo updated.")
            st.rerun()
        except Exception as error:
            st.error(f"Could not save logo: {error}")
...
if "Dashboard" in menu:
    ...
    with updates_col:
        ...
        project_updates_df = fetch_project_updates(current_project)
        render_project_updates_slider(project_updates_df, height=260)
        if not project_updates_df.empty:
            with st.expander("💬 Comments", expanded=False):
                update_labels = {
                    f"Update #{row.get('id')} · {str(row.get('caption', 'Project update'))[:32]}": row.get('id')
                    for _, row in project_updates_df.iterrows()
                }
                selected_label = st.selectbox("Choose update", list(update_labels), key="comment_update")
                selected_id = update_labels[selected_label]
                comments_df = fetch_update_comments(selected_id)
                if comments_df.empty:
                    st.caption("No comments yet. Start the conversation.")
                else:
                    for _, comment in comments_df.tail(4).iterrows():
                        st.markdown(f"**{html.escape(str(comment.get('author_name', 'Visitor')))}** · {str(comment.get('created_at', ''))[:10]}")
                        st.caption(str(comment.get('comment_body', '')))
                commenter = st.text_input("Your name", key="comment_author")
                new_comment = st.text_area("Add a comment", key="comment_body", height=80)
                if st.button("Post comment", use_container_width=True, key="post_comment"):
                    if new_comment.strip():
                        try:
                            add_update_comment(selected_id, commenter, new_comment)
                            st.rerun()
                        except Exception as error:
