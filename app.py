...
# --- 2. PDF GENERATION (Jaisa aapne manga tha) ---
def export_to_pdf(dataframe, title):
    ...
    data = [["Date", "Item Name", "Amount", "Detail"]]
...
st.markdown("""
    ...
    div[data-testid="stMetric"] { background-color: #f8f9fa; border-radius: 15px; padding: 15px; }
    .header-box { text-align: center; background: #1e1e1e; padding: 30px; border-radius: 20px; border-bottom: 5px solid #FF4B4B; }
    </style>
...
if menu == "📊 Dashboard":
    ...
    if not df.empty:
        ...
        m1.metric("💰 Income", f"PKR {inc:,.0f}")
        m2.metric("📉 Expenses", f"PKR {exp:,.0f}")
        m3.metric("⚖️ Balance", f"PKR {inc-exp:,.0f}")
    ...
    if "show_form" in st.session_state and is_auth:
        ...
        with st.expander(f"Add {ftype}", expanded=True):
            with st.form("entry_form"):
                ...
                uploaded_photo = None
                if ftype == "Material":
                    uploaded_photo = st.file_uploader("Upload Bill Image", type=['jpg', 'png', 'jpeg'])
        ...
        if search and not f_df.empty and 'image_url' in f_df.columns:
            for _, row in f_df.iterrows():
                if row['image_url'] and str(row['image_url']) != "nan":
                    with st.expander(f"🖼️ Bill Photo (ID: {row['id']})"):
                        st.image(row['image_url'], use_container_width=True)

        st.metric("Total PKR", f"{f_df['amount'].sum():,.0f}")
        st.divider()
        c1, c2 = st.columns(2)
        c1.download_button("📥 Excel", f_df.to_csv().encode('utf-8'), "Report.csv")
        c2.download_button("📄 PDF Report", export_to_pdf(f_df, menu), "Report.pdf")
