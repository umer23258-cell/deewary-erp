...
if menu == "📊 Dashboard":
    ...
    if "show_form" in st.session_state:
        if is_auth:
            ...
            if is_editing and not df.empty:
                edit_row = df[df['id'] == st.session_state.edit_id]; row = edit_row.iloc[0]
            ...
            with st.expander(f"{'Edit' if is_editing else 'New'} {st.session_state.show_form} Entry", expanded=True):
                with st.form("entry_form"):
                    d_date = st.date_input("Date", defaults["date"]); d_name = st.text_input("Name / Description", defaults["name"]); d_amt = st.number_input("Amount", min_value=0.0, value=defaults["amount"]); d_det = st.text_area("Details", defaults["detail"])
                    d_occ, d_rec, d_meth = "", "", ""
                    if st.session_state.show_form == "Labor":
                        col_a, col_b, col_c = st.columns(3); d_occ = col_a.text_input("Occupation", defaults["occ"]); d_rec = col_b.text_input("Received By", defaults["rec"]); d_meth = col_c.selectbox("Method", ["Cash", "Online"], index=0 if defaults["meth"] == "Cash" else 1)
                    if st.form_submit_button("Update Record" if is_editing else "Save to Cloud"):
                        ...
                        try:
                            ...
                            st.cache_data.clear(); [st.session_state.pop(k) for k in ["show_form", "edit_id"] if k in st.session_state]; st.success("Transaction Synced!"); st.rerun()
                        except Exception as e: st.error(f"Error: {e}")
            if st.button("❌ Close Form"): [st.session_state.pop(k) for k in ["show_form", "edit_id"] if k in st.session_state]; st.rerun()
    ...
    with about_col1:
        st.markdown("""
        **Deewary.com** aik premium Real Estate aur Construction firm hai jo modern architecture aur quality construction mein maharat rakhti hai. Hamara maqsad logon ko unke khuwabon ka ghar tameer kar ke dena hai, jahan mazbooti aur khoobsurti ka haseen imtizaj ho.
        
        **Hum Kya Karte Hain?**
        *   **Real Estate:** Behtareen locations par plots aur houses ki khareed-o-faroakht.
        *   **Construction:** 5 Marla se le kar 1 Kanal tak ke houses ki A+ quality construction.
        *   **Renovation:** Purane gharon ko modern designs mein tabdeel karna.
        *   **Project Management:** Har project ko digital tools (ERP) ke zariye transparent banana.
        
        *Hum sirf deewary nahi tameer karte, hum aapka mustaqbil banate hain.*
        """)
    ...
    with about_col2:
        ...
        <div style="background-color: #1E1E1E; padding: 20px; border-radius: 15px; color: white; border: 1px solid #4B4B4B;">
            ...
            <p style="font-size: 14px;">"Construction industry mein transparency aur technology la kar client ka bharosa jeetna hamari pehli tarjeeh hai."</p>
            <hr style="border: 0.5px solid #4B4B4B;">
            <p style="font-size: 13px;"><b>Experience:</b> 5+ Years<br>
            <b>Completed Projects:</b> 20+<br>
            <b>Location:</b> Rawalpindi/Islamabad</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with supp_col1:
        ...
        st.info("Yeh digital portal Deewary.com ke har project ki financial transparency aur records ko maintain karne ke liye design kiya gaya hai. Har entry cloud par encrypted hoti hai.")
    with supp_col2:
        st.subheader("🛠️ Support")
        whatsapp_url = "https://wa.me/923115190118"
        st.markdown(f"""
        **Developer:** umer sherin  
        **Contact:** <a href="{whatsapp_url}" target="_blank" style="text-decoration:none; color:#25D366; font-weight:bold;">💬 Chat on WhatsApp</a>  
        **Status:** Active ✅
        """, unsafe_allow_html=True)
        ...
        if is_auth:
            ...
            if c_ed.button("✏️ Edit"):
                if target_id in filtered_df['id'].values:
                    row = filtered_df[filtered_df['id'] == target_id].iloc[0]; st.session_state.show_form = row['type']; st.session_state.edit_id = target_id; st.success("ID Loaded! Dashboard check karein."); st.rerun()
                else: st.error("ID nahi mili.")
            if c_de.button("🗑️ Delete"):
                if target_id != 0: supabase.table('transactions').delete().eq('id', target_id).execute(); st.cache_data.clear(); st.success("Deleted!"); st.rerun()
