...
def export_to_pdf(dataframe, title):
    ...
    elements.append(Paragraph(f"<font color='#1e3a8a' size=18><b>{title}</b></font>", styles['Title']))
...
def export_labor_profile_pdf(labor_row, payments_df):
    ...
    # Header
    elements.append(Paragraph(f"<font color='#0f172a' size=22><b>DEEWARYN.COM ERP</b></font>", styles['Title']))
    elements.append(Paragraph(f"<font color='#1e3a8a' size=14><b>LABOR PROFILE DOSSIER & LEDGER REPORT</b></font>", styles['Normal']))
    elements.append(Paragraph(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M')}", styles['Normal']))
    ...
    elements.append(Paragraph("<font color='#0f172a' size=12><b>💵 STATEMENT OF PAID PAYMENTS HISTORY</b></font>", styles['Heading2']))
...
# --- 3. PAGE CONFIG ---
st.set_page_config(page_title="Deewaryn.com ERP", layout="wide", page_icon="🏗️", initial_sidebar_state="expanded")

# --- ✨ HIGH-END LUXURY REAL ESTATE THEME DESIGN (CSS INJECTION) ✨ ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap');
    
    /* Pure Modern Background Reset */
    ...
    /* Premium Pill-Style Navigation Tabs */
    ...
    /* Beautiful Premium Website Header */
    .premium-header {
        text-align: center;
        background: #ffffff;
        padding: 45px 25px;
    ...
    /* Clean Cards for Financials/Metrics */
    .beauty-card {
        background: #ffffff;
        padding: 24px;
        border-radius: 20px;
    ...
    /* Minimalist Elegant Table Wrapper */
    .stDataFrame {
...
def check_password():
    ...
    with st.sidebar.expander("🔐 Admin Access Pin", expanded=True):
...
# --- 8. SIDEBAR DESIGN (Clean Luxury White Layout) ---
with st.sidebar:
    st.markdown("<h2 style='color:#1e3a8a; font-weight:800; margin-bottom:0; font-size:24px; letter-spacing:-0.5px;'>DEEWARYN</h2>", unsafe_allow_html=True)
    st.markdown("<p style='font-size:11px; color:#64748b; font-weight:500; margin-top:2px; text-transform:uppercase; letter-spacing:1px;'>Corporate Portal ERP</p>", unsafe_allow_html=True)
    st.divider()
    ...
    if is_auth:
        st.markdown("<p style='font-size:11px; font-weight:700; color:#166534; text-transform:uppercase; margin-bottom:8px;'>⚡ Admin Actions</p>", unsafe_allow_html=True)
...
if "Dashboard" in menu:
    st.markdown(f"""
        <div class="premium-header">
            <h1 style="color: #0f172a; margin: 0; font-weight:800; letter-spacing: -0.5px; font-size:36px;">DEEWARYN<span style="color:#1e3a8a;">.COM</span></h1>
            <p style="color: #64748b; letter-spacing: 0.5px; font-size: 14px; margin: 6px 0 18px 0; font-weight:500;">PREMIUM SYSTEM INTERFACE • SITE CONTEXT: {current_project.upper()}</p>
            <div style="background: rgba(30, 58, 138, 0.06); color: #1e3a8a; display: inline-block; padding: 6px 20px; border-radius: 30px; font-weight: 700; font-size: 13px; border: 1px solid rgba(30, 58, 138, 0.15);">
    ...
    with col_left:
        st.markdown(f"### 📈 Construction Milestone Progress")
        st.progress(prog_val / 100)
        st.markdown(f"**{prog_val}% Tasks Complete**")
    ...
    with col_right:
        st.markdown("### 📝 Architectural Checklist")
        st.write(f"✅ Cleared Status Tasks: **{done_tasks}** | ⏳ Pending Tasks: **{total_tasks - done_tasks}**")
        if st.button("Re-Sync Ledger Cache", use_container_width=True): st.cache_data.clear(); st.rerun()

    st.divider()
    st.markdown("### 🏗️ Site Checklist Matrix")
    t_cols = st.columns(3)
...
# --- ISOLATED INDEPENDENT PAGE: 📑 RECEIPT VOUCHER SYSTEM ---
    ...
    st.write("Dynamic transaction invoices styled beautifully for client review.")
    st.divider()
    ...
    if not df.empty:
        ...
        st.markdown(f"""
            <div style="background-color: #ffffff; border: 1px solid #e2e8f0; padding: 35px; border-radius: 24px; max-width: 550px; margin: 20px auto; color: #0f172a; box-shadow: 0 20px 40px rgba(0,0,0,0.03);">
                <div style="text-align: center; border-bottom: 1px solid #f1f5f9; padding-bottom: 18px; margin-bottom: 22px;">
                    <h3 style="margin: 0; color: #0f172a; letter-spacing: -0.5px; font-weight:800; font-size:22px;">DEEWARYN<span style="color:#1e3a8a;">.COM</span></h3>
                ...
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color:#475569;"><span>Voucher Reference ID:</span><b style="color:#1e3a8a; font-weight:700;">{v_number}</b></div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color:#475569;"><span>Execution Log Date:</span><span style="color:#0f172a; font-weight:500;">{v_row['date']}</span></div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color:#475569;"><span>Ledger Allocation:</span><b style="color:#0f172a;">{str(v_row['type']).upper()}</b></div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color:#475569;"><span>Particular Scope:</span><b style="color:#0f172a;">{v_row['name']}</b></div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color:#475569;"><span>Designation Spec:</span><span style="color:#0f172a; font-weight:500;">{v_row.get('occupation', 'N/A') if pd.notna(v_row.get('occupation')) else 'N/A'}</span></div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 14px; color:#475569;"><span>Disbursed/Authorized:</span><span style="color:#0f172a; font-weight:500;">{v_row.get('received_by', 'N/A') if pd.notna(v_row.get('received_by')) else 'N/A'}</span></div>
                <div style="display: flex; justify-content: space-between; margin-bottom: 18px; font-size: 14px; color:#475569;"><span>Channel Pipeline:</span><span style="color:#0f172a; font-weight:500;">{v_row.get('pay_method', 'Cash') if pd.notna(v_row.get('pay_method')) else 'Cash'}</span></div>
                <p style="font-size: 13px; background: #f8fafc; padding: 14px; border-radius: 12px; font-style: italic; border-left: 4px solid #1e3a8a; margin-bottom:24px; color:#475569;">Memo Notes
