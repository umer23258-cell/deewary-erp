...
st.markdown("""
    ...
    [data-testid="stMainBlockContainer"] {
        background: rgba(248, 250, 252, 0.92) !important; /* White smooth opaque look over the image */
        border-radius: 24px;
...
if "Dashboard" in menu:
    ...
    if not labor_df.empty:
        ...
        pdf_bytes = export_labor_profile_pdf(l_row, p_history)
        st.download_button("📥 Download Labor Ledger Dossier (PDF)", data=pdf_bytes, file_name=f"Labor_Dossier_{selected_labor}.pdf", mime="application/pdf")
    else:
        st.info("No profile structures synchronized inside this site scope registry.")

elif "Search & Audit Reports" in menu:
    st.subheader("🔍 Master Audit Ledger & PDF Document Center")
    if not df.empty:
        st.dataframe(df[['id', 'date', 'type', 'name', 'amount', 'pay_method', 'detailCenter']], use_container_width=True)
        
        pdf_report = export_to_pdf(df, f"Master Audit Report: {current_project}")
        st.download_button("📥 Generate Operational PDF Report", data=pdf_report, file_name=f"ERP_Audit_Report_{current_project}.pdf", mime="application/pdf")
    else:
        st.info("Database matrix contains no operational data to download.")
